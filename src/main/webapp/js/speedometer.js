...
function Speedometer(Element) {
  ...
  var ThreshPivot = options.thresholdPivot || 0.0;
  var ThreshWidth = options.thresholdWidth || 50;
  ...
  this.drawBackground = function ()
  {
    ...
    context.lineWidth = Size / ThreshWidth;


    var top = (MaxValue - MinValue )/2
    var size = MaxValue - MinValue
    var sizePercent = (Threshold-MinValue)/size *100;  // (0--40)/80*100   = 50%
    var thresholdAngle = (MeterRimAngle)*(sizePercent/100);   //(45--225)*0,5

    var startValuePercent = (ThreshPivot-MinValue) / size*100-sizePercent/2;
    var startAngle = MeterFromAngle+(MeterRimAngle * startValuePercent / 100);


    if(startAngle<MeterFromAngle) {
        thresholdAngle-=MeterFromAngle- startAngle;
        startAngle=MeterFromAngle;
    }

    if ((startAngle+thresholdAngle)>MeterToAngle) {
        thresholdAngle=MeterToAngle-startAngle;
    }


    context.strokeBoxedArc (x + gap, y + gap, w - gap * 2, h - gap * 2,
                            TBE.Deg2Rad (startAngle), TBE.Deg2Rad (thresholdAngle),
                            /* counterclockwise = */ false);
  }
}; // End of class
